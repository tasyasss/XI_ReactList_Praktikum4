					...
					{this.state.siswa.map((item,index) => {
						return (
							<li className="list-group-item" key={index}>
								...
								<button className="btn btn-sm btn-primary m1" onClick={() => this.Edit(item)} data-toggle="modal" data-target="#modal">
									Edit
								</button>
								<button className="btn btn-sm btn-danger m1" onClick={() => this.Drop(index)}>
									Hapus
								</button>
							</li>
						);
						...
						<div className="modal-content">
							...
							<form onSubmit={this.SaveSiswa}>
								<div className="modal-body">
									NIS
									<input type="text" name="nis" className="formcontrol" onChange={this.bind}
									value={this.state.nis} />
									Nama
									<input type="text" name="nama" className="formcontrol" onChange={this.bind}
									value={this.state.nama} />
									Alamat
									<input type="text" name="alamat" className="formcontrol" onChange={this.bind}
